...
from llm_files.llm_functions import classify_topic_by_title, score_complexity_by_markdown, search_news_articles, classify_arguments_by_markdown, write_summary_by_markdown
                        ...
                        for chapter in erlaeuterung['erlaeuterung']['kapitel']:
                            ...
                            for komponente in chapter['komponenten']:
                                ...
                                if komponente['typ'] == "text":
                                    ...
                                    sentences = [s + '.' for s in re.split(r'\.\s', komponente['text']['text']) if s]
                                    # Entferne den zusätzlichen Punkt vom letzten Satz, falls vorhanden
            ...
            for chapter in content_dict.get("kapitel", []):
                ...
                for component in chapter.get("komponenten", []):
                    ...
                    if comp_type == "title":
                        ...
                        if img_url:
                            img_url = build_vote_url(voteId, img_url, "llm_files/.env")
                        ...
                        if url:
                            url = build_vote_url(voteId, url, "llm_files/.env")
...
def classify_vote(voteId):
    ...
    try:
        ...
        # key_to_check = "6710"
        if voteId in data:
            return data[voteId]['voteCategory']
        else:
...
def build_vote_url(vote_id: int, file_name: str = "erlaeuterung.json", dotenv_path="llm_files/.env") -> str:
...
def build_votes_url(dotenv_path="llm_files/.env") -> str:
