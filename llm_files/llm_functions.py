...
def score_complexity_by_markdown(client, path_to_markdown="../markdown_output/erlaeuterungen_6770_de.md", retry=""):
    ...
    # TODO Rephrase this prompt that the model assess the level of difficulty to understand the contents being an average citizen. Wording and Concept.
    prompt = f"""{retry} The attached markdown file contains all the information that was given to Swiss citizens to vote on this topic. Score the complexity by labelling it "easy", "normal", or "difficult". Consider that this score needs to apply for an average citizen. For example, the popular vote about the "Bundesgesetz vom 17. Juni 2016 über steuerliche Massnahmen zur Stärkung der Wettbewerbsfähigkeit des Unternehmensstandorts Schweiz (Unternehmenssteuerreformgesetz III)" was very difficult to unterstand as it was a very law specific matter, on the other side the idea of the popular vote "Volksinitiative vom 21. März 2011 «1:12 - für gerechte Löhne»" was rather easy to grasp. Only output the label of the score, no reasoning at all.
    ...
    try:
        ...
        print("Response with ", client, "for ", path_to_markdown, response.content)
        if not response.content or not response.content.strip():
...
class Sentence(BaseModel):
    ...
    def text_hash(self) -> str:
         return hashlib.md5(self.sentence.encode('utf-8')).hexdigest()
...
def classify_arguments_by_markdown(markdown_path):
    ...
    try:
        ...
        return vote_instance.model_dump(by_alias=True)
